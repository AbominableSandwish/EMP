...
namespace emp {
	void Camera::SetProjection(float angle) {
		...
		projection = glm::perspective(glm::radians(angle), (float)(m_graphic->width/ delta) / (float)m_graphic->height, 0.1f, 1000.0f);
	}

	void Camera::Reset(){
		this->projection = glm::mat4(1.0f);
		float delta = (float)m_graphic->width / (float)m_graphic->height;
		projection = glm::perspective(glm::radians(this->angle_proj), (float)(m_graphic->width / delta) / (float)m_graphic->height, 0.1f, 1000.0f);
	}
