...
void Hierarchy::Draw()
{
	ImGui::Begin("Hierarchy", &is_open, ImGuiWindowFlags_MenuBar);
	...
	if (entities->size() != 0) {
		...
		for (auto entity : *entities)
		{
			...
			if (ImGui::TreeNode(name.c_str()))
			{
				...
				{
					ImGui::SetDragDropPayload("spore", entity.name.c_str(), sizeof(const char*));
			...
			else
			{
				...
				if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(1))
					ImGui::OpenPopup(("context_menu" + entity.name).c_str());

				if (ImGui::BeginPopup(("context_menu" + entity.name).c_str())) {
